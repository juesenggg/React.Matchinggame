...
class App extends Component {
  ...
  cardPressHandler = (index) => {
    ...
    if (this.state.firstPickedIndex == null && this.setState.secondPickedIndex == null){
      this.setState({
        isOpen,
        ...
        <SafeAreaView style={ styles.container }>
          ...
          <View style={ styles.main }>
            <View style={ styles.gameBoard }>
              ...
              <Card key={index} onPress={ () => this.cardPressHandler(index) } style={ styles.button } fontSize={22} title={symbol} cover="⁇" isShow={this.state.isOpen[index]} />
              )}
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  button:{
    ...
    justifyContent: 'center',
    alignItems: 'center',
    margin:(Dimensions.get('window').width - (30 * 6)) / (7 * 2),
  },
  tryAgainButton:{
    backgroundColor:'#eee',
    padding: 8,
